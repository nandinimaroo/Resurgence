...
if(GameStats.Lives>0)
  ...
  bool RandomPoint(Vector3 center, float range, out Vector3 result)
    ...
    if(RandomPoint(gameObject.transform.position,20f,out spawnPosition))
	{
...
public int ZombiesCaptured=0;
	...
	IEnumerator MeeleAttackWeaponHit(){
		if (Physics.Raycast (ray1, out hitInfo, 2f, ~ignoreLayer) || Physics.Raycast (ray2, out hitInfo, 2f, ~ignoreLayer) || Physics.Raycast (ray3, out hitInfo, 2f, ~ignoreLayer)
			...
			if (hitInfo.transform.tag=="TreatableZombie") {
				Transform _other = hitInfo.transform.root.transform;
				if (_other.transform.tag == "TreatableZombie") {
						GameObject g =_other.transform.gameObject;
				Animator anim=g.GetComponent<Animator>();
				anim.SetTrigger("IsFalling");
				Destroy(g,0.2f);
				ZombiesCaptured++;
				GameStats.CaptureScore=ZombiesCaptured;
				}
